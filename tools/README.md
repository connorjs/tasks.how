...
For the TypeScript side specifically, Cerberus now treats `pnpm-lock.yaml` as the contract and lets Bazel materialize hermetic npm trees only for the actions that need them. That keeps the repo free of ambient `node_modules` state while still leaving Prettier and ESLint easy to run in CI.
